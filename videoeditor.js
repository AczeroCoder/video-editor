...
const closeIconHTML = `
<svg viewBox="0 0 48 48" height="24" width="24" fill="currentColor">
...
class RenderedLayer {
    ...
    init(player, preview) {
        ...
        delete_option.innerHTML = closeIconHTML;
...
class AudioLayer extends RenderedLayer {
     ...
     */
     render(ctx_out, ref_time) {
        ...
        if (!this.started) {
            if (!this.source) {
                init_audio(ref_time); // ERROR / init_audio is not defined
            }
...
class Player {
    ...
    scrubMove(ev) {
        ...
        if (this.selected_layer) {
            ...
            // ERROR / CANNOT DIVIDE TIME IF IT IS REALLY SMALL
        ...
        this.cursor_preview.style.bottom = (rect.height + 10) + "px";
    ...
    resize_time() {
        this.time_canvas.style.width = this.time_holder.clientWidth * this.time_scale + 'px';
...
function exportVideo(blob) {
    ...
    const vidSrc = URL.createObjectURL(blob);
...
$("#addImageBtn").click(function() {
    let x = getCookie("session_id");
    if (x) {
        // OPEN UPLOAD DIV
        console.log("open div from design.js")
        upload();
    } else {
        upload();
    }
